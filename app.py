...
def buscar_jogos(data_str):
    ...
    if 'response' not in response or not response['response']: return pd.DataFrame()
        ...
        if not opcoes.empty:
            ...
            if st.session_state.get('mostrar_resultados', False):
                with st.spinner('Analisando dados...'):
                    ...
                    l_m, l_v = calcular_medias_ponderadas(j_d['ID_Mandante'], 'home'), calcular_medias_ponderadas(j_d['ID_Visitante'], 'away')
                    ...
                    # --- VOLTANDO OS GRÁFICOS E CONFIANÇA ---
